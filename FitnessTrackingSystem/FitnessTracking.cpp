...
public:
	...
	void accept_node(string name, long step_goal1, int water_intake_goal,
			int sleep_goal, int i);
	long step_goal(long);
	float calories();
...
void list::accept_node(string name, long step_goal1, int water_intake_goal,
		int sleep_goal, int i) {                    //accepting node data fields
	node *temp, *ptr;
	long steps_walked;
...
float list::calories()  //Counts calories based on food had in 24 hrs
{
	...
	do { //Generalised food option categories having average calories per quantity eaten
		cout << "FOOD OPTIONS:\n1.Pulse/Grains?" << endl;
		cout << "2.Fruits?" << endl;
		...
		switch (a) {     //assigning calories to meal eaten
		case 1:
			if (a == 1) {
				cal1 = 200;
			}
...
void list::display()  //displaying details of all day
{
	...
	line();
	cout << "DAY" << setw(15) << "WATER INTAKE" << setw(15) << "SLEEP TIME "
			<< setw(15) << "STEPS" << setw(20) << "WORKOUT" << setw(20)
			<< "CALORIES\n";
	line();
	for (ptr = head; ptr != NULL; ptr = ptr->next) {
		if (ptr->day == 0) {
			...
			cout << ptr->day << setw(12) << ptr->no_of_glasses << setw(12)
					<< ptr->sleep_count << setw(20) << ptr->step_count
					<< setw(20) << ptr->workout << setw(20) << ptr->cal << endl;
		}

	}
}
void list::display1(string name, int age, char gender, float height,
		float weight, float bmi, int sleep_goal, int water_intake_goal,
		long step_goal1) {   //display function for displaying details of person
	...
	cout << name << "\t\t" << age << "\t\t" << gender << "\t\t" << height
			<< "\t\t" << weight << "\t\t\t" << sleep_goal << "\t\t\t"
			<< water_intake_goal << "\t\t\t" << step_goal1 << "\t\t" << bmi
			<< endl;
	cout
			<< "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
			<< endl;
...
//display average analysis
		...
		cout
				<< "-----------------------------------------------------------------------------------------------------------------------------------------"
				<< endl;
		cout << avg_step_count << setw(30) << avg_cal << setw(30)
				<< avg_sleep_count << setw(30) << avg_no_of_glasses << endl;
		cout
				<< "------------------------------------------------------------------------------------------------------------------------------------------"
				<< endl;
				...
				do {
					...
					case 2:
						cout << "\nEnter updated water intake in glasses "
								<< endl;
						cin >> no_of_glasses1;
						ptr->no_of_glasses = ptr->no_of_glasses
								+ no_of_glasses1; //adding no of glasses to existing glass count
...
int main() {
	...
	do {
		...
		case 1:
			...
			cin >> name;
			do {
				for (int k = 0; name[k] != '\0'; k++) { //validation for name starts
					if (!((name[k] >= 'a' && name[k] <= 'z') || (name[k] == 39)
							|| name[k] >= 'A' && name[k] <= 'Z')) {
						flag7 = 1;
		...
		case 2:
			...
			l.display1(name, age, gender, height, weight, bmi1, sleep_goal,
					water_intake_goal, step_goal1);
			cout << "HEALTH STATUS:";
			l.bmi12(bmi1);

			break;
		case 3:
			cout << "Enter number of days for which you want to enter data"
					<< endl;
			cin >> no_of_days;
			...
			for (int i = 1; i <= no_of_days; i++)  //displaying data
					{
				cout << "DAY:" << i << endl;
				l.accept_node(name, step_goal1, water_intake_goal, sleep_goal,
						i); //changed,,passing i
				cout << endl;
			}
			break;
